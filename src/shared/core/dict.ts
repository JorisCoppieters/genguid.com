...
export function resolveKeyValues(in_dict: any) {
    return Promise.all(_toEntries(in_dict).map((entry) => (_isPromise(entry.value) ? entry.value : Promise.resolve(entry.value)).then((value: any) => _keyValObj(entry.key, value)))).then(
        _fromEntries
    );
}
...
export function getOrderedValues(in_dictionary: { [key: string]: any }, in_castFunction: (value: any) => void) {
