...
alert_user "${APP_NAME}" "Starting database deploy to ${ENV_TYPE_FULL} now..."
...
if [[ -d "./src/server/scripts/migration" ]]; then
    cp -r "./src/server/scripts/migration"/* "${DIST_FOLDER}"
...
DIST_COUNT=$(ls -1 "${DIST_FOLDER}" | wc -l)
...
if [[ $DIST_COUNT -gt 0 ]]; then
    if [[ "${ENV_TYPE}" != "dev" ]]; then
        ...
        echo ""
        echo "#"
        echo "# Zipping dist & creating scripts"
        echo "#"
        echo ""
        # set -x
        ...
        cd "${DIST_FOLDER}"
        zip -r "${DIST_ZIP}" *
        cp -r "../tools/scripts/deploy-database-remote.sh" "${REMOTE_SCRIPT}"
...
alert_user "${APP_NAME}" "Database deploy to ${ENV_TYPE_FULL} complete!"